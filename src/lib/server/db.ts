...
export async function initDatabase(): Promise<void> {
  ...
  initializationPromise = dbInitializationLock.withLock(async () => {
    ...
    try {
      ...
      console.log(`🚀 Base de données prête pour la recherche en ${Math.round(duration / 1000)}s (${conversations.length} conversations indexées)`);
...
export async function searchConversations(
  ...
  try {
    ...
    console.log(`📊 Configuration de l'index:`, {
      tokenize: 'forward',
      resolution: 9,
      fields: ['conversation_a.content', 'conversation_b.content', 'short_summary', 'keywords']
    });
    ...
    console.log(`📊 Diagnostic: 5 premières conversations IDs:`, conversations.slice(0, 5).map(c => c.id));
    ...
    // Diagnostic: Tester aussi avec la version en minuscules
    console.log(`🔍 Diagnostic: Test recherche avec minuscules...`);
    const searchResultsLower = index.search(queryLower, {
      limit: 1000,
      enrich: true
    });
    
    console.log(`📊 Diagnostic: Résultats avec casse originale: ${searchResults.length}`);
    console.log(`📊 Diagnostic: Résultats avec minuscules: ${searchResultsLower.length}`);
    
    console.log(`📊 Résultats bruts de FlexSearch:`, {
      count: searchResults.length,
      firstResults: searchResults.slice(0, 3).map((r: any) => ({ id: r.id, score: r.score }))
    });
    
    // Transformer les résultats en SearchResult
    console.log(`🔄 Transformation des ${searchResults.length} résultats bruts...`);
    const allResults: SearchResult[] = searchResults.map((result: any) => {
      const conv = conversations.find(c => c.id === result.id);
      if (!conv) {
        console.log(`⚠️ Conversation non trouvée pour l'ID: ${result.id}`);
        return null;
      }
